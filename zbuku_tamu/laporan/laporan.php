...
$tamu_terakhir = mysqli_query($conn, "SELECT *, `tanggal/waktu` AS tanggal FROM tamu ORDER BY `tanggal/waktu` DESC LIMIT 1");
$tamu = mysqli_fetch_assoc($tamu_terakhir);

// --- Ambil data grafik per bulan ---
$grafik = mysqli_query($conn, "
    SELECT MONTH(`tanggal/waktu`) AS bulan, COUNT(*) AS jumlah
    FROM tamu
    WHERE YEAR(`tanggal/waktu`) = '$tahun_filter'
    GROUP BY MONTH(`tanggal/waktu`)
    ORDER BY MONTH(`tanggal/waktu`)
");
...
<div class="container">
    ...
    <?php if ($tamu): ?>
        ...
        <p><strong>Instansi:</strong> <?= htmlspecialchars($tamu['instansi']) ?></p>
        <p><strong>Keperluan:</strong> <?= htmlspecialchars($tamu['keperluan']) ?></p>
        <p><strong>Tanggal:</strong> <?= htmlspecialchars($tamu['tanggal']) ?></p>
    <?php else: ?>
