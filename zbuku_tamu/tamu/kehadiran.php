...
// Hitung total data
$sql_count = "SELECT COUNT(*) as total FROM tamu WHERE YEAR(`tanggal/waktu`) = $tahun_filter";
if (!empty($search)) {
...
// Ambil data
$sql_data = "SELECT * FROM tamu WHERE YEAR(`tanggal/waktu`) = $tahun_filter";
if (!empty($search)) {
    $sql_data .= " AND nama LIKE '%$search%'";
}
$sql_data .= " ORDER BY `tanggal/waktu` DESC LIMIT $limit OFFSET $offset";
